...
>   - **ÖRNEK ALANI ≠ KOPYALANACAK İÇERİK:** Few-shot örnek "masraf (8503)", bu agent dosyasındaki [A12]-[A18] inline örnekleri ise "Döviz Alarm" alanındandır. **İkisi de yalnızca stil/derinlik/format göstergesidir.** Agent gerçek projeye uyarlar; masraf veya döviz alarm metinlerini, ekran adlarını veya kararlarını çıktıya **kopyalamaz** (uydurma sayılır, [A6.1]).
...
4. Yazılımın Fonksiyonel Gereksinimleri
   4.1 Yazılım İşlevleri
      (Aşağıdaki sıra, karar matrisi sırasıyla ([A9.1] + template "4.1 Özet Karar Matrisi") birebir aynıdır.
       Numaralar REFERANS sıradır; gerçek alt başlık numarası [A9.2] gereği yalnızca "Evet" satırlara
       ardışık atanır — yani bir işlevde atlanan "Hayır" satırlar numara almaz, sabit .1–.11 kullanılmaz.)
...
> **"Başlık silinmez" kuralının kapsamı (KRİTİK AYRIM):** Bu kural **doküman bölüm/alt başlıklarına** uygulanır — yani template'teki 3.4.x, 4.2.x, 4.4.x, 5.x gibi sabit başlıklar etki yoksa bile silinmez, altına [C15] standart cümlesi yazılır. **Ancak 4.1.X karar matrisi satırlarına uygulanmaz:** orada özet-merkezli yaklaşım ([A9.1.1]) geçerlidir — "Hayır" satırlar her işlev altında yazılmaz, yalnızca 4.1 sonundaki tek "Özet Karar Matrisi"nde topluca yansır. Kısaca: **başlık = kalır; işlev-altı Hayır matris satırı = yazılmaz.**
...
| 2 | Karar matrisi ↔ detay | Her "Evet" satırının altında detay alt başlığı (4.1.X.n) olmalı; "Hayır" satırlar **işlev altında yazılmaz**, yalnızca 4.1 sonundaki tek **Özet Karar Matrisi**'nde topluca yansır ([A9.1.1]). NOT: "başlık silinmez" kuralı doküman bölüm başlıklarına (3.4.x/4.2/4.4/5.x) aittir, işlev-içi matris satırlarına değil — bkz. [A3] kapsam notu |
...
### [A7.2] Veri Güvenliği — Çıktıya Hassas Veri Yazma YASAĞI (ZORUNLU)

> Bu agent mssql (VpStringResource, MobileMenu, log tabloları), Confluence ve codebase okur. Çıktı dokümanı kullanıcıya verilen bir deliverable'dır; içine **gerçek hassas veri sızmamalıdır.**

- **Gerçek müşteri PII YAZILMAZ:** ad-soyad, TCKN, telefon, e-posta, hesap/kart numarası, bakiye, adres vb. Log/contact-history şeması anlatılırken **alan adı/şema** yazılır, gerçek değer değil. Örnek gerekiyorsa maskeli/uydurma örnek kullanılır (örn. kart `**** **** **** 1234`, TCKN `***`).
- **Sır / kimlik bilgisi YAZILMAZ:** connection string, DB kullanıcı/şifre, API token, secret key, internal host/IP. Sorgu örnekleri verilirken bağlantı bilgisi gizlenir.
- **mssql sorgu sonuçları:** Yalnızca **şema/metadata** (kolon adı, tip, ResourceKey, MenuID gibi tanım verisi) dokümana girer; **müşteri satırları (data rows) dokümana kopyalanmaz.** Gerekirse `SELECT TOP 0` / sadece `INFORMATION_SCHEMA` ile yapı çekilir.
- **Resource değerleri istisnadır:** VpStringResource'taki UI metinleri (tr/en/ar) müşteriye gösterilen genel metinlerdir, PII değildir — 3.4.5 CMS tablosuna yazılabilir.
- **Self-review taraması (Adım 4'e dahil):** Çıktıda TCKN/kart no/telefon/e-posta deseni, `Password=`, `Server=`, `token`/`secret` gibi kalıplar taranır; bulunursa maskelenir veya kaldırılır ve completeness raporuna not düşülür.
...
1. **Default akış:** Her 4.1.X altında **tam 11 satırlı matris yazılmaz.** Yalnızca **"Evet" işaretlenen satırlar** kısa bir tablo ile (3 sütun: Başlık / Durum / Not) yazılır. "Hayır" satırlar işlev altında **yazılmaz** ve "başlık silinmez" kuralına tabi değildir (o kural yalnızca doküman bölüm başlıklarına aittir — bkz. [A3] kapsam notu); Hayır satırlar yalnızca 4.1 sonundaki Özet Karar Matrisi'nde topluca yansır.
...
> **[A11.1] (onay) ↔ [A19] (karar sorusu) ayrımı:** Bunlar **ortogonaldir.** [A19] *karar/içerik* sorularını fazlar (ne yapılacak); [A11.1] üretilmiş bölümün *onayını* alır (doğru yazılmış mı). İkisi karıştırılmaz. Yorgunluğu azaltmak için onaylar da [A19.2] ruhuna uyar: çok işlevli projede her küçük alt başlık için ayrı onay sorulmaz; onay yalnızca büyük bölümler (Bölüm 1, 3.4 grubu, her 4.1.X) için alınır, kullanıcı "tümünü sonda" derse tek toplu onaya iner.
...
> ## TEK KAYNAK İLKESİ (Single Source of Truth) — [A12]–[A18]
>
> [A12]–[A18] arasındaki disiplinler (UI yerleşim, else-case/hata, muğlak ifade, müşteri yolculuğu, form validasyon, servis sözleşmesi, pilot/HPC) bu dokümanın **kanonik kural tanımlarıdır.** Aşağıdaki yerler bu kuralları **yeniden tanımlamaz, yalnızca atıf yapar:**
> - **[A6.2] Cross-reference tablosu** → her satır ilgili [A1x] kuralının *denetimidir* (kuralın kendisi değil).
> - **4.1.X İşlev Yazım Şablonu (8 madde)** → her madde ilgili [A1x] kuralını *uygular* ([A12]/[A13]/[A16]/[A17]/[A15] referanslarıyla).
> - **BÖLÜM BAZLI DOLDURMA KURALLARI** → bölüm-özgü doldurma talimatı; ortak disiplin için [A1x]'e atıf yapar.
>
> **Çelişki çözümü:** Herhangi iki yer arasında ifade farkı olursa **[A12]–[A18]'deki tanım esastır.** İçerik güncellenirken kural tek yerde (ilgili [A1x]) değiştirilir; denetim/şablon/bölüm metinleri yalnızca atfı korur. Bu, dosya boyutundan kaynaklı talimat seyrelmesini (instruction dilution) ve stale referans riskini azaltır.
...
### [A12.5] Erişilebilirlik (Engelsiz Bankacılık) Entegrasyonu

> **Tetik:** 3.4.2 Engelsiz Bankacılık = **Evet** olduğunda bu alt başlık zorunludur; aksi halde tek satır "Erişilebilirlik etkisi 3.4.2'de Yok olarak işaretlenmiştir." yeterlidir.

3.4.2 = Evet ise, her yeni UI bileşeninin ([A12]) ve form alanının ([A16]) yanında **erişilebilirlik notu** yazılır:

| Erişilebilirlik Bilgisi | Ne Yazılır |
|-------------------------|------------|
| **Ekran okuyucu etiketi (accessibility label)** | Bileşenin VoiceOver/TalkBack'te okunacak metni (resource key 3.4.5'te; metin tam Türkçe) |
| **Odak sırası (focus order)** | Yeni bileşen mevcut odak akışına nereye giriyor (örn. "başlıktan sonra, ilk karttan önce") |
| **Dokunma hedefi / kontrast** | Min. dokunma alanı ve metin/arka plan kontrast gereksinimi karşılanıyor mu ([BELIRSIZ] ise tasarım ekibine sorulur) |
| **Durum bildirimi** | Hata/başarı/yükleniyor durumları ekran okuyucuya nasıl duyurulur (örn. inline hata metni `accessibilityLiveRegion`) |

> Bilgi kapsam/Figma annotation'ında yoksa uydurulmaz: `[BELIRSIZ — erişilebilirlik tasarım ekibiyle netleştirilecek]` + completeness notu. Erişilebilirlik metinleri de [A7.2] ve 3.4.5 CMS akışına tabidir (resource key 4.1.X içinde geçmez).

### [A12.6] Arapça (ar-SA) RTL Notu

> 3.4.5'te ar-SA dili dolduruluyorsa ([İngilizce/Arapça menü] = Evet), Arapça **sağdan-sola (RTL)** bir dildir. Yeni UI bileşeninin yerleşim açıklamasında ([A12.2]) yön-bağımlı bir ifade varsa ("sağda", "soldaki ikonun yanında") RTL'de **aynalanacağı** kısaca not edilir (örn. "ikon başlangıç/leading kenarda — RTL'de sağda görünür"). Yön-bağımsız ("üstte/altta") yerleşimlerde not gerekmez. Detaylı RTL davranışı teknik tasarıma bırakılır; analiz dokümanında yalnızca aynalama olduğu işaretlenir.
...
Her 4.1.X iskeletinin sonunda **8. bölüm** olarak "Sonraki Adım / İlişkili İşlevler" satırı yer alır (4.1.X yazım şablonunun 8 maddesiyle birebir; bkz. "4.1.X İşlev Yazım Şablonu"):
...
### [A16.5] Form Erişilebilirliği

> 3.4.2 Engelsiz Bankacılık = Evet ise her form alanı için [A12.5] erişilebilirlik notu (ekran okuyucu etiketi, odak sırası, hata duyurusu) zorunludur; özellikle inline hata metinlerinin ekran okuyucuya nasıl duyurulduğu belirtilir.
...
> **TEK KAYNAK (Single Source of Truth):** Bu tablo, "4.1 Derinleştirme Soruları" (13 soru) ve 4.3 loglama sorularının **tek render hedefidir.** İlgili kararlar [A19.1] Faz B'de **bir kez** sorulur; cevaplar buraya işlenir. Aynı konu (pilot, HPC, eski client, force update, TrackMobileEvent, SAS, Dataroid, Adjust, kart maskeleme, İng/Ar dil) 4.3 loglama bölümünde veya işlev altında **tekrar sorulmaz** ([A19.3]); 4.3 yalnızca bu tablodaki loglama satırlarına atıf yapar.
...
## [A19] SORU PLANI VE SORU BÜTÇESİ (Soru Yorgunluğunu Önleme)

> Bu agent doğası gereği çok soru sorar (geliştirme tipi, kaynak, granülerlik, matris stratejisi, akış tipi, 3.3 adayları, 3.4.1-3.4.4, her giriş noktası için [A12.1], 13 derinleştirme, 4.3 loglama, bölüm onayları). Plansız sorulduğunda çok işlevli bir projede 30-50+ AskUserQuestion turu oluşur ve kullanıcı yorulur. [A19] bu çağrıları **planlı, toplu turlara** indirger.

### [A19.1] Üç Soru Aşaması (Faz)

Sorular dağınık sorulmaz; üç faza toplanır:

1. **Faz A — Proje açılışı (1 kez, en başta):** Geliştirme tipi (0.5) + kaynak toplama (0.6) + granülerlik (0.9) + matris stratejisi ([A9.1.1]). Bunlar tüm doküman tonunu belirleyen tek seferlik kararlardır; tek blokta toplanır (AskUserQuestion 4 soru sınırına göre).
2. **Faz B — Kapsam geneli kararlar (1 kez, 4.1 öncesi):** 13 derinleştirme sorusu ([A18.1] tablosunu besler) + 4.3 loglama + 3.4.1-3.4.4 etki kararları. Bunlar **işlev başına değil, kapsam genelinde bir kez** sorulur ([A19.3] tekilleştirme). AskUserQuestion 4 soru / 4 seçenek sınırına göre cascade gruplanır (modül 06 [C6.1]).
3. **Faz C — İşlev bazlı kararlar (her 4.1.X için, yalnızca gerçekten işleve özgü olanlar):** [A12.1] yerleşim (yalnızca o işlevde yeni UI giriş noktası varsa), [A13.3] maks sınır (yalnızca liste/sayaç varsa), 3.3 kapsam-dışı adayı (tarama bulguları). İşleve özgü olmayan hiçbir soru burada tekrar sorulmaz.

### [A19.2] Soru Bütçesi (Hedef)

- **Faz A:** ≤ 1 tur. **Faz B:** ≤ 3 tur (cascade). **Faz C:** işlev başına ≤ 2 tur.
- Bütçe aşılacaksa: ilgili soruları tek AskUserQuestion içinde çoklu `questions` (maks 4) ile birleştir; gerekirse `[BELIRSIZ]` + completeness raporuna taşı, kullanıcıyı bölme.
- Kullanıcı Faz A'da "Sadece kritik maddeler" (granülerlik) seçtiyse Faz B/C soruları yalnızca kritik işlevler için sorulur; gerisi default + `[BELIRSIZ]`.

### [A19.3] Tekilleştirme (Aynı Soruyu İki Kez Sorma)

- Bir karar **bir kez** sorulur ve cevabı `docs/.mobile-00-state.json` `cevaplar` altına yazılır. Aynı konu başka bir bölümde gerektiğinde state'ten okunur, tekrar sorulmaz.
- Özellikle SAS loglama, pilot, eski client, İng/Ar dil etkisi **hem 13 derinleştirmede hem 4.3/3.4'te** geçer; bunlar [A18.1] tablosunun kaynağıdır ve yalnızca Faz B'de bir kez sorulur (bkz. [A18.1] not).
- Kullanıcı bir tercihi (modül 13) daha önce belirttiyse (örn. matris stratejisi) tekrar sorulmaz; tercih uygulanır.
...
| 3.4.2 Engelsiz Bankacılık | "Var / Yok" sor. Yok ise "Internet veya Mobil uygulamalara etkisi yoktur." Var ise HPC + sözleşme içeriyor mu tablosu. **Var ise [A12.5] zorunlu:** her yeni UI bileşeni ve form alanında erişilebilirlik notu ([A12]/[A16] entegrasyonu) yazılır. |
...
Bu bölüm aşağıdaki sorularla netleşir. **Bu 13 soru [A19.1] Faz B'de KAPSAM GENELİNDE bir kez sorulur** (işlev başına tekrarlanmaz); cevaplar [A18.1] Derinleştirme Tablosuna işlenir ve aynı konular 4.3 loglama / işlev altında tekrar sorulmaz ([A19.3] tekilleştirme). Yalnızca gerçekten işleve özgü farklılık varsa (örn. bir işlevde segment farkı diğerinde yok) o işlev için ek soru sorulur (Faz C):
...
Loglama detayı net değilse [A14.3]: **"Loglama detayı teknik tasarım aşamasında netleştirilecektir."** Yanına gerekçe (örn. "EDW ekibi alan listesini iletecek"). Asla "ihtiyaç olarak işaretlenmiştir" cümlesiyle kapatılmaz.

- **4.3.1 Ürün İşlem log / Müşteri log / ADK log / Kullanıcı log / Arcsight / Teftiş log + EDW Extra Field + Contact History:**

> **Tekilleştirme ([A19.3]):** Aşağıdaki loglama soruları [A19.1] Faz B'de 13 derinleştirme sorusuyla **aynı turda** sorulur; SAS loglama ve EDW/Contact History 13 derinleştirmede (madde 8-9) zaten alındıysa **tekrar sorulmaz**, state'ten okunup [A18.1] tablosuna işlenir. Aşağıdaki blok yalnızca bu konular Faz B'de henüz sorulmadıysa kullanılır.
...
1. `docs/.mobile-00-state.json` var mı kontrol et.
   ...
   - **Kullanıcının cevabı Adım 1'i belirler:** "Devam" → mevcut `docs/mobile-sdlc-analiz.md` KORUNUR (Write ile ezilmez). "Baştan" → kullanıcı açıkça onayladıktan sonra dosya yeniden oluşturulabilir.
...
#### Adım 0.7.1: Kaynaksız Degrade-Mod (Saf Röportaj Fallback)

> **Ne zaman:** Confluence + kapsam dökümanı + Figma'nın **üçü de "yok"** olduğunda (yalnızca codebase/DB erişilebilir veya hiçbiri yok). Bu modda [A6.1] kanıt-dayanaklı disiplin gereği çoğu alan otomatik doldurulamaz; agent **uydurmaz**, bunun yerine yapılandırılmış röportaja geçer.

1. **Kullanıcıyı bilgilendir (uyarı, suçlama değil):** "Tasarım/kapsam kaynağı paylaşılmadığı için ekran metinleri (3.4.5 CMS), Figma frame referansları ([A8]) ve bazı 4.1.X detayları kaynaktan doğrulanamayacak. İki seçenek var:"

```
AskUserQuestion(
  questions: [{
    question: "Kaynak olmadan nasıl ilerleyelim?",
    header: "Degrade-Mod",
    multiSelect: false,
    options: [
      { label: "Saf röportaj (Önerilen)", description: "Her bölümü ben anlatırım; agent yazıya döker, kaynak gerektiren alanları [BELIRSIZ — kaynak bekleniyor] işaretler" },
      { label: "İskelet + sonra kaynak", description: "Sadece başlık iskeleti + default cümleler üretilsin; kaynak gelince doldurulsun" },
      { label: "Kaynağı şimdi paylaşacağım", description: "Adım 0.6'ya dön" }
    ]
  }]
)
```

2. **Saf röportaj seçilirse:** Agent codebase/DB'den (semantic-search + mssql) çekebildiği kadarını çeker; geri kalan her alanı [A1] röportaj moduyla kullanıcıdan **düz metin** olarak alır. Kaynak gerektiren ve röportajla da netleşmeyen alanlar (resource key değerleri, Figma frame, kesin servis sözleşmesi) `[BELIRSIZ — kaynak bekleniyor]` + completeness raporuna taşınır; **doküman içine "Confluence paylaşılmadığı için..." gibi meta-cümle YAZILMAZ** ([A14.1]).
3. **Codebase de yoksa:** Yalnızca Bölüm 1 + default cümleler + tamamen röportaja dayalı iskelet üretilir; doküman "taslak (kaynak doğrulaması bekliyor)" olarak işaretlenir ve completeness skoru düşük raporlanır.
...
- **OVERWRITE GUARD (ZORUNLU — veri kaybını önler):** Write işleminden önce dosya zaten var mı kontrol et.
  - Dosya VAR + Adım 0'da kullanıcı **"Devam"** dediyse → **Write YAPMA.** Mevcut dokümanı koru; yalnızca `[BEKLENIYOR]` bölümleri Edit ile doldurmaya devam et.
  - Dosya VAR + kullanıcı **"Baştan"** dediyse → yalnızca **açık onaydan sonra** template ile yeniden oluştur (eski içerik kaybolacak uyarısı verilir; istenirse `docs/mobile-sdlc-analiz.{tarih}.bak.md` yedeği alınır).
  - Dosya YOK → template ile oluştur.

### Adım 2: Bölümleri Doldur (BÖLÜM BAZLI DOLDURMA KURALLARI'na göre)

Template sırasıyla her bölüm için, bu agent dosyasındaki **"BÖLÜM BAZLI DOLDURMA KURALLARI"**nı uygula. **Sorular [A19] Soru Planına göre fazlanır:** Faz A açılışta (0.5/0.6/0.9 + matris stratejisi), Faz B 4.1 öncesi kapsam-geneli kararlar (13 derinleştirme + 4.3 loglama + 3.4.1-3.4.4, bir kez), Faz C yalnızca işleve özgü kararlar. Aynı soru iki kez sorulmaz ([A19.3]).
...
- **Sorulacak bölümler** (3.3, 3.4.1-3.4.4, 4.1 derinleştirme 13 sorusu, 4.3): AskUserQuestion (modül 06 şeması) ile [A19] fazlamasına göre sor, cevaba göre doldur.
...
- **State:** `docs/.mobile-00-state.json` (ilerleme + `mcp_cache` [A5.4] + `cevaplar` [A19.3]) | **Completeness:** `docs/.mobile-00-completeness.md`.
- **Kaynak digest ([A5.1]):** `docs/.mobile-00-kaynak-ozet.json` — okunan Confluence/kapsam/Figma/MCS verisinin yapısal özeti (ham içerik context'te tutulmaz).
- **Rolling summary ([A5.5]):** `docs/.mobile-00-rolling-summary.md` — bölüm grubu özetleri (context yönetimi).
- **Çok işlevli (3+) çıktı ([A5.3]):** `docs/mobile-sdlc-analiz/index.md` + `docs/mobile-sdlc-analiz/4.1.X-{slug}.md`.
- **Yedek (overwrite guard, Adım 1):** `docs/mobile-sdlc-analiz.{tarih}.bak.md` (yalnızca "baştan" + onay durumunda).

> Tüm `.mobile-00-*` dosyaları **iç/state dosyalarıdır** (kullanıcı deliverable'ı değil); yalnızca `docs/mobile-sdlc-analiz.md` (veya çok işlevli klasör) kullanıcıya verilir.
